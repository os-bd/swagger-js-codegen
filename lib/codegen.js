  ...
  _.forEach(swagger.paths, function(api, path) {
    ...
    _.forEach(api, function(op, m) {
      ...
      var method = {
        ...
        headers: [],
        responses: {},
        actionName: String(path + "/" + M).replace(/\/([a-z])/gi, function(
          all,
          letter
        ) {
      ...
      _.forEach(responses, function(response, status) {
        let responseStatus = (method.responses["status" + status] = {
          tsType: ts.convertType(response)
        });
        if (responseStatus.tsType.isRef === true) {
          responseStatus.tsType["properties"] =
            swagger.definitions[responseStatus.tsType.target].properties;
          //qiyu 追踪data结构中引用类型的数据结构，再次解析，用于判断是否包含rows属性
          if (
            responseStatus.tsType["properties"].data !== undefined &&
            responseStatus.tsType["properties"].data.$ref !== undefined
          ) {
            var refSegments = responseStatus.tsType[
              "properties"
            ].data.$ref.split("/");
            ...
            _.forEach(swagger.definitions, function(
              ...
              if (definitionName === name) {
                var property = ts.convertType(definition, swagger);
                responseStatus.tsType["properties"] = property;
              }
            });
          }
          //end
