...
exports.tpl = {
  ...
  */
  ...
  public async {{&actionName}} ( {{#parameters}}{{#isQueryParameter}}{{safeName}}:{{#tsType}}{{> type}}{{/tsType}},{{/isQueryParameter}}{{#isBodyParameter}}{{safeName}}:{{#tsType}}{{> type}}{{/tsType}},{{/isBodyParameter}}{{#isPathParameter}}{{safeName}}:{{#tsType}}{{> type}}{{/tsType}},{{/isPathParameter}}{{/parameters}} ):Promise< {{#responses}}{{#status200}}{{#tsType}}{{> type}}{{/tsType}}{{/status200}}{{/responses}} > {
      const result: {{#responses}}{{#status200}}{{#tsType}}{{> type}}{{/tsType}}{{/status200}}{{/responses}} = await this.http.get("{{{path}}}", { {{#parameters}}{{#isQueryParameter}}{{safeName}}:{{safeName}},{{/isQueryParameter}}{{#isBodyParameter}}{{safeName}}:{{safeName}},{{/isBodyParameter}}{{/parameters}} });
      return result;
  }
  {{/isGET}}
  {{^isGET}}
  public async {{&actionName}} ( {{#parameters}}{{#isQueryParameter}}{{safeName}}:{{#tsType}}{{> type}}{{/tsType}},{{/isQueryParameter}}{{#isBodyParameter}}{{safeName}}:{{#tsType}}{{> type}}{{/tsType}},{{/isBodyParameter}}{{#isPathParameter}}{{safeName}}:{{#tsType}}{{> type}}{{/tsType}},{{/isPathParameter}}{{/parameters}} ):Promise< {{#responses}}{{#status200}}{{#tsType}}{{> type}}{{/tsType}}{{/status200}}{{/responses}} > {
      const result: {{#responses}}{{#status200}}{{#tsType}}{{> type}}{{/tsType}}{{/status200}}{{/responses}} = await this.http.post(\`{{{path}}}?{{#parameters}}{{#isQueryParameter}}{{/isQueryParameter}}{{#isQueryParameter}}{{safeName}}=\${ {{safeName}} }&{{/isQueryParameter}}{{/parameters}}\`, {{#parameters}} {{#isBodyParameter}} {{safeName}},{{/isBodyParameter}} {{/parameters}} );
      return result;
  }
  {{/isGET}}`,
  methodType: `{{#tsType}}
